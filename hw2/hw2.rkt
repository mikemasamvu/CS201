...
; ********************************************************
; CS 201 HW #2  DUE Wednesday 2/22/2023, 11:59 pm
...
(define (depth tree)
  (cond ((null? tree) 0)
        ((not(list? tree)) 0)
        (else (max (+ 1 (depth (car tree))) (depth (rest tree))))
  ))
...
(define (multiply x y)
  (cond
    ...
    ((and (not (number? x)) (number? y)) y)
    ((and (not (number? y)) (number? x)) x)
    ((and (not (number? x)) (not (number? y))) 'none)))

;; multiply allows only 2 numbers to multiply themselves and prevents other data types e.g (multiply 3 6) gives 18 and (multiply a b) gives 'none

(define (prod tree)
  (cond
    ((null? tree) 'none)
    ((list? (car tree)) (multiply (prod (car tree)) (prod (rest tree))))
    (else
     (multiply (car tree) (prod (rest tree))))
  ))
...
(define (count-if pred tree)
  (cond
    [(null? tree) 0]
    [(list? (car tree)) (+ (count-if pred (car tree)) (count-if pred (rest tree)))]
    [else
     (if (pred (car tree))
         (+ 1 (count-if pred (rest tree)))
         (+ 0 (count-if pred (rest tree))))]  
  ))
;(trace count-if)
...
;; types-helper checks for the datatype of the input e.g (types-helper 4) gives 'integer and (types-helper "a") gives string

(define (listcontrol tree)
  (cond
    [(not (list? tree)) (list tree)]
    [(empty? tree) empty]
    [else
     (append (listcontrol (car tree)) (listcontrol (rest tree)))]))

;; listcontrol converts all inputs that into single list e.g (listcontrol 4) gives '(4) and (listcontrol '()) gives '()

(define (types tree)
  (sort (remove-duplicates (listcontrol(cond
    [(null? tree) empty]
    [(list? (car tree)) (cons (types (car tree)) (types (rest tree)))]
    [else
     (cons (types-helper (car tree))  (types (rest tree)))]
    ))) symbol<?))
...
(define (hextree tree)
  (cond
    [(empty? tree) '()]
    [(not (list? (car tree)))
      (if (equal? #f (assoc (car tree) dectohexalist))
          (cons (car tree) (hextree (rest tree)))
          (cons (cadr (assoc (car tree) dectohexalist)) (hextree (rest tree)))
          )]
    [else (cons (hextree (car tree)) (hextree (rest tree)))]
...
(define (minvalue x y)
  (cond
    [(and (number? x) (number? y)) (min x y)]
    [(number? x) x]
    [(number? y) y]
    [else empty]))

;; minvalue calculates minimum value between two inputs even if one of the inputs is not an integer e.g (minvalue 2 3) gives 3 and (minvalue x y) gives '()
  
(define (tree-min tree)
  (cond
    [(empty? tree) empty]
    [(not (list? (car tree))) (minvalue (car tree) (tree-min (rest tree)))]
    [else
     (minvalue (tree-min (car tree)) (tree-min (rest tree)))]
  ))
...
(define (map-tree proc tree)
  (cond
    [(empty? tree) '()]
    [(not (list? (car tree))) (cons (proc (car tree)) (map-tree proc (rest tree)))]
    [else
     (cons (map-tree proc (car tree)) (map-tree proc (rest tree)))])
  )
